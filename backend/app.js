...
app.post('/inserirOng', autenticarToken, async (req, res) => {
    try {
        ...
        const sqlOng = `INSERT INTO ong (id_usuario, nome, email, telefone, endereco, cnpj, cidade, estado, pix, trabalho_ofe) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
        ...
        res.json({
            resposta: "ONG cadastrada com sucesso!",
            id_ong: id_ong
        });
    } catch (error) {
        console.error("ERRO NO BANCO:", error);
        res.status(500).json({ erro: "Erro ao salvar: verifique se as colunas batem com o banco." });
    }
});

// BUSCAR ONGS DO USUÁRIO
app.get('/minhas-ongs', autenticarToken, async (req, res) => {
    try {
        const id_usuario = req.user.id_usuario;

        const sql = `
            SELECT ong.id_ong, ong.nome, ong.cidade, ong.email, ong.cnpj
            FROM ong
...
app.get('/mostrarAnimal', async (req, res) => {
    try {
        ...
        res.json(resultado); // Retorna o array de animais
    } catch (error) {
        console.error(error);
...
// ================= INSERIR ONG =================
app.post('/inserirOng', autenticarToken, async (req, res) => {
    // Usar o pool diretamente é mais simples e evita esquecer de dar 'release'
    try {
        const { nome, telefone, endereco, cidade, estado, pix, trabalho_ofe, cnpj } = req.body;
        const id_usuario = req.user.id_usuario;

        // --- IMPORTANTE: Verifique se no Heidi o nome é 'cadastro_ong' ou 'ong' ---
        // Vou usar 'cadastro_ong' que é o padrão que combinamos antes
        const [resultado] = await pool.query(
            `INSERT INTO cadastro_ong (nome, telefone, endereco, cidade, estado, pix, trabalho_ofe, cnpj) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
            [nome, telefone, endereco, cidade, estado, pix || null, trabalho_ofe || null, cnpj]
        );

        const id_ong = resultado.insertId;

        // 2. Criar vínculo (Aqui o ID do usuário que veio do Token se liga à nova ONG)
        await pool.query(
        ...
        // Se o erro for 'Table doesn't exist', o console vai te avisar qual nome está errado
        if (error.code === 'ER_NO_SUCH_TABLE') {
            return res.status(500).json({ erro: "Tabela não encontrada no banco de dados." });
        }

        res.status(500).json({ erro: "Erro interno ao cadastrar ONG" });
    }
});


// ================= ATUALIZAR ONG =================
app.put('/atualizarOng', autenticarToken, async (req, res) => {
    try {
        const { id_ong, nome, telefone, endereco, cidade, estado, pix, trabalho_ofe, cnpj } = req.body;
        const id_usuario = req.user.id_usuario;

        // Verifica se a ONG pertence ao usuário
        const [verifica] = await pool.query(
            `SELECT * FROM usuario_ong WHERE id_usuario = ? AND id_ong = ?`,
            [id_usuario, id_ong]
        );

        if (verifica.length === 0) {
            return res.status(403).json({ erro: "Você não tem permissão para editar essa ONG" });
        }

        const [resultado] = await pool.query(
            `UPDATE ong 
             SET nome = ?, telefone = ?, endereco = ?, cidade = ?, estado = ?, pix = ?, trabalho_ofe = ?, cnpj = ?
             WHERE id_ong = ?`,
            [nome, telefone, endereco, cidade, estado, pix, trabalho_ofe, cnpj, id_ong]
        );

        if (resultado.affectedRows === 1) {
            return res.json({ resposta: "ONG atualizada com sucesso!" });
        }
...
app.delete('/deletarOng/:id', autenticarToken, async (req, res) => {
    try {
        ...
        if (verifica.length === 0) {
            return res.status(403).json({ erro: "Sem permissão para deletar essa ONG" });
        }

        // Deleta vínculo primeiro (boa prática)
        await pool.query(`DELETE FROM usuario_ong WHERE id_ong = ?`, [id_ong]);
...
app.get('/minhas-ongs', autenticarToken, async (req, res) => {
    try {
        ...
        const [ongs] = await pool.query(
            `SELECT o.id_ong, o.nome, o.cidade, o.estado
             FROM ong o
             INNER JOIN usuario_ong uo ON o.id_ong = uo.id_ong
             WHERE uo.id_usuario = ?`,
            [id_usuario]
        );
...
app.post('/inserirDica', autenticarToken, async (req, res) => {
    try {
        ...
        const [resultado] = await pool.query(sql, [
            id_usuario, titulo, conteudo, categoria, imagem, data_publicacao
        ])

        return res.json({
            resposta: resultado.affectedRows === 1
                ? "Dica cadastrada com sucesso!"
                : "Erro ao cadastrar dica!"
        })
...
app.put('/atualizarDica', autenticarToken, async (req, res) => {
    try {
        ...
        const [resultado] = await pool.query(sql, [
            id_usuario, titulo, conteudo, categoria, imagem, data_publicacao, id_dica
        ])

        if (resultado.affectedRows === 1) {
            return res.json({ resposta: "Dica atualizada com sucesso!" })
        }

        return res.status(404).json({ erro: "Dica não encontrada ou nada foi alterado" })
...
app.delete('/deletarDica', autenticarToken, async (req, res) => {
    try {
        ...
        const [resultado] = await pool.query(`DELETE FROM dicas WHERE id_dica = ?`, [id_dica])
...
app.get('/mostrarEventos', async (req, res) => {
    try {
        const [resultado] = await pool.query(`SELECT e.*, o.nome AS ongFROM eventos eJOIN ong o ON e.id_ong = o.id_ong`)
...
app.post('/inserirEvento', autenticarToken, async (req, res) => {
    try {
        ...
        const [resultado] = await pool.query(sql, [
            id_ong, nome_evento, hora, data, endereco, categoria, localizacao
        ])
...
app.put('/atualizarEvento', autenticarToken, async (req, res) => {
    try {
        ...
        const [resultado] = await pool.query(sql, [
            id_ong, nome_evento, hora, data, endereco, categoria, localizacao, id_evento
        ])
